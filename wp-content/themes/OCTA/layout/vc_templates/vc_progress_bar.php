 ...
 */
$title = $values = $units = $badg = $barsstyle = $bgcolor = $css = $custombgcolor = $customtxtcolor = $options = $el_class = $css_animation = '';
$output = '';
...
if($barsstyle == 'lg-line inner-title'){
    $badg = ' class="sm-badge badge"';
}else if($barsstyle == 'tiny-line tiny-2'){
    $badg = ' class="sm-badge sm-progress"';
} else {
    $badg = ' class="sm-badge"';
}

$class_to_filter = 'progress-bars vc_progress_bar wpb_content_element';
$class_to_filter .= vc_shortcode_custom_css_class( $css, ' ' ) . $this->getExtraClass( $el_class ) . ' '.$barsstyle;
...
foreach ( $graph_lines_data as $line ) {
    ...
    if($barsstyle != 'lg-line inner-title'){
        $output .= '<span class="bar-title">'. $line['label'].'</span>';
    }
	
    $output .= '<div class="vc_general vc_single_bar progress' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ? ' vc_progress-bar-color-' . $line['color'] : '' ). '">';
	
        //$output .= '<small class="vc_label"' . $line['txtcolor'] . '>' . $unit . '</small>';
	    if ( $max_value > 100.00 ) {
        ...
        $output .= '<span class="vc_bar progress-bar ' . esc_attr( implode( ' ', $bar_options ) ) . '" data-percentage-value="' . esc_attr( $percentage_value ) . '" data-value="' . esc_attr( $line['value'] ) . '"' . $line['bgcolor'] . '>';
        
        if($barsstyle == 'tiny-line'){
            $output .= '<span'.$badg.'>';
                $output .= $unit;
            $output .= '</span>';
        }
