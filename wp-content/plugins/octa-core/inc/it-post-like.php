...
function like_scripts() {
    wp_enqueue_script( 'jm_like_post', plugin_dir_url( __FILE__ ) .'js/post-like.min.js', array('jquery'), '1.0', 1 );  
...
function getPostLikeLink( $post_id ) {
    ...
    $output = '<span class="hidden lk">'.__('Like','superfine').'</span><span class="hidden unlk">'.__('Unlike','superfine').'</span><a href="#" class="jm-post-like'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'">'.$heart.'&nbsp;'.$count.'</a>';
    return $output;
}     
...
add_action( 'admin_init', 'import_settings' );
if ( ! function_exists( 'import_settings' ) ) {
    function import_settings() {
        if (isset($_POST['upload_options'])) {
            if ($_FILES["import_file"]["error"] > 0) {
                die();
            } else {
                $fileName = $_FILES["import_file"]["tmp_name"];
                $options = unserialize(file_get_contents($fileName));
                if ($options) {
